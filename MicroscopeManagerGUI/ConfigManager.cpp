		...
		while (getline(file, line))
		{
			...
			if (line == "CAMERA_SETTINGS")
			{
				while (getline(file, line))
				{
					...
					if (cameraSettingType == "Integer")
					{
						...
						if (cameraSettingName == "Width")
						{
							width = intValue;
						}
						else if (cameraSettingName == "Height") 
						{
							height = intValue;
						}
					}
			...
			if (line == "ODORANT_SETTINGS")
			{
				while (getline(file, line))
				{
					...
					// odorants index + new line character
					odorants.push_back(atoi(line.c_str()));
			...
			//createimagemanager after reading all of the settings
			//mm->CreateImageManager(imageManager, volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, width, height);
			...
			if (line == "STATE_SETTINGS")
			{
				while (getline(file, line))
				{
					...
					/*stateAndDuration.push_back(atoi)*/
					...
					state = (char)(atoi(line.c_str()) >> 16);

					getline(file, line);
					duration = atoi(line.c_str());
					stateAndDuration.push_back(std::pair<char, int>(state, duration));
...
void ConfigManager::GetExperimentSettings(float* vsMin, float* vsMax, int* fpv, int* vps, int* lm, float* sa, std::string* exp, std::string* imMan, std::vector<char>* odr, std::vector<std::pair<char,int>>*sAndD)
{
	//if (volumeScaleMin != -1)
	//{
