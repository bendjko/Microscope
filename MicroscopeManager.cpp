...

{
	...
	{
		imageManager = new RawImageManager(filename_,minVol, maxVol, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, width, height);
	}
	...
	{
		imageManager = new TIFFImageManager(filename_, minVol, maxVol, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, width, height);
	}
...
void MicroscopeManager::WriteFile(unsigned char* buf, unsigned long long writeSize, MICROSCOPE_METADATA* metabuffer, bool newImage /*=false*/, bool process /*=false*/, bool compress /*=false*/)
{
	if (imageManager)
	{
		if (process)
		...
		imageManager->WriteFile(buf, writeSize, metabuffer, newImage);
	}
}
