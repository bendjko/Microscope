...
public:
	...
	void CreateImageManager(std::string imageManagerType, float minVol, float maxVol, int framesPerVolume, int volumesPerSecond, int laserMode, float scannerAmplitude, std::vector<char> odorants, int width, int height);
	...
	void OpenFile();
	void CloseFile();
	void WriteFile(unsigned char* buf, unsigned long long writeSize, MICROSCOPE_METADATA* metabuffer, bool newImage = false, bool process = false, bool compress = false);
