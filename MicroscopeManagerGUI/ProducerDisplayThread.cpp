...
ProducerDisplayThread::ProducerDisplayThread(int bufsize, MicroscopeManager* mm, /*MICROSCOPE_METADATA*md,*/ int planeCount, float volumeRangeMin, float volumeRangeMax, int laserMode, char sampleInfo[478], int volumesPerSecond, std::vector<std::pair<char, int>>stateAndDuration, std::vector<char>odorants, QObject* mainWindow, int* targetFrameInfo) :
	ProducerThread(bufsize, mm,/* md,*/ planeCount, volumeRangeMin, volumeRangeMax, laserMode, sampleInfo, volumesPerSecond, stateAndDuration, odorants),
