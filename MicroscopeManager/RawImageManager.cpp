...
RawImageManager::RawImageManager(std::string filename, float minVol, float maxVol, int slicesPerVol, int volumesPerSecond, int laserMode, float scannerAmplitude, std::vector<char> odorants, unsigned long width, unsigned long height) :
	filename_(filename),
	file_(0),
	open_(false)
{
...
void RawImageManager::WriteFile(unsigned char* buf, unsigned long long writeSize, MICROSCOPE_METADATA* metabuffer, bool newImage)
{
	if (open_)
	{
		DWORD bytesWritten;
