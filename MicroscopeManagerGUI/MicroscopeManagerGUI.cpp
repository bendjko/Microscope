    ...
    scannerAmplitude(1)
{
    ...
    //float frameRate = mm->GetCameraFloatParameter(REMOTE_MODULE, "AcquisitionFrameRate");
    ...
    targetFrameInfo = new int[3];
    targetFrameInfo[0] = framesPerVolume;
    targetFrameInfo[1] = 0;
    targetFrameInfo[2] = volumesPerSecond;
...
void MicroscopeManagerGUI::readConfig()
{
    ...
    else
    {
        ...
        cfg.GetExperimentSettings(&volumeScaleMin, &volumeScaleMax, &framesPerVolume, &volumesPerSecond, &laserMode, &scannerAmplitude, &experimentSettingsDevice, &imageManagerType, &odorants, &stateAndDuration);
    }
}

void MicroscopeManagerGUI::snapImage() // potential error since imagemanager is not initialized and writefile may not have access to experiment variables
{
    if (filepath == "")
    {
        setFilename();
    ...
    img = QImage(buf, width, height, width, QImage::Format_Grayscale8, NULL, NULL); //image buffer
...
void MicroscopeManagerGUI::acquireStart()
{
    ...
    cameraThd = new ProducerThread(static_cast<int>(mm->GetCameraIntParameter(STREAM_MODULE, "Width")* mm->GetCameraIntParameter(STREAM_MODULE, "Height")), mm, /*md,*//*, this, targetFrameInfo*/ framesPerVolume, volumeScaleMin, volumeScaleMax, laserMode, sampleInfo, volumesPerSecond, stateAndDuration, odorants);

    ImageManager* imageManager = (ImageManager*)VirtualAlloc(NULL, sizeof(ImageManager*), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
    if (imageManagerType == "Raw")
    {
        imageManager = new RawImageManager(filepath, volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, mm->GetCameraIntParameter(STREAM_MODULE, "Width"), mm->GetCameraIntParameter(STREAM_MODULE, "Height"));
    }
    else if (imageManagerType == "TIFF")
    {
        imageManager = new TIFFImageManager(filepath, volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, mm->GetCameraIntParameter(STREAM_MODULE, "Width"), mm->GetCameraIntParameter(STREAM_MODULE, "Height"));

    }

    else if (imageManagerType == "HDF5")
    {
        imageManager = new HDF5ImageManager(filepath, volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, mm->GetCameraIntParameter(STREAM_MODULE, "Width"), mm->GetCameraIntParameter(STREAM_MODULE, "Height"));
...
void MicroscopeManagerGUI::experimentSetup()
{
    ...
    if (!acquiring && !experimentActive)
    {
        ...
        command = "";
        command += VOLUME_SCALE_MIN + std::to_string(volumeScaleMin) + '\r'; // galvo = minVol in microscope management
        mm->SerialWrite(experimentSettingsDevice, command.c_str(), command.size());
...
void MicroscopeManagerGUI::startExperiment()
{
    if (experimentActive && !acquiring)
    {
       ...
       /* mm->SetFilename(filepath);
        mm->CreateFile();
        MICROSCOPE_METADATA* md;*/
        cameraThd = new ProducerDisplayThread(1920*1080, mm, /*md,*/ framesPerVolume, volumeScaleMin, volumeScaleMax, laserMode, sampleInfo, volumesPerSecond, stateAndDuration, odorants, this, targetFrameInfo); // , stateAndDuration, odorants, true, framesPerVolume, volumesPerSecond, volumeScaleMin, volumeScaleMax, laserMode, laserPower, experimentDescription);
       /* ProducerDisplayThread(int bufsize, MicroscopeManager * mm, int planeCount, float volumeRangeMin, float volumeRangeMax, int laserMode, char sampleInfo[478], int volumesPerSecond, std::vector<std::pair<char, int>>stateAndDuration, std::vector<char>odorants, QObject * mainWindow, int* targetFrameInfo);*/
        ...
        if (laserMode == 0) //Both lasers
        {
            for (int laser = 1; laser <= 2; laser++)
            {
                for (int frame = 1; frame <= framesPerVolume; frame++)
                {
                    ...
                    if (imageManagerType == "Raw")
                    {
                        //imageManager = new RawImageManager(filename, volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, mm->GetCameraIntParameter(STREAM_MODULE, "Width"), mm->GetCameraIntParameter(STREAM_MODULE, "Height"));
                       imageManager = new RawImageManager(filepath,volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, mm->GetCameraIntParameter(STREAM_MODULE, "Width"), mm->GetCameraIntParameter(STREAM_MODULE, "Height"));

                    }
                    else if (imageManagerType == "TIFF")
                    {
                        //imageManager = new TIFFImageManager(filename, volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode,scannerAmplitude, odorants, mm->GetCameraIntParameter(STREAM_MODULE, "Width"), mm->GetCameraIntParameter(STREAM_MODULE, "Height"));
                        imageManager = new TIFFImageManager(filepath, volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, mm->GetCameraIntParameter(STREAM_MODULE, "Width"), mm->GetCameraIntParameter(STREAM_MODULE, "Height"));
                    ...
                    {
                        // imageManager = new HDF5ImageManager(filename, volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, mm->GetCameraIntParameter(STREAM_MODULE, "Width"), mm->GetCameraIntParameter(STREAM_MODULE, "Height"));
                        imageManager = new HDF5ImageManager(filepath, volumeScaleMin, volumeScaleMax, framesPerVolume, volumesPerSecond, laserMode, scannerAmplitude, odorants, mm->GetCameraIntParameter(STREAM_MODULE, "Width"), mm->GetCameraIntParameter(STREAM_MODULE, "Height"));
        ...
        {  
            for (int frame = 1; frame <= framesPerVolume; frame++)
            {
                ...
                new WriterThread((ProducerThread*)cameraThd, imageManager); //writing thread to camera
...
{
    std::vector<char> testOdorants;
    mm->CreateImageManager("HDF5", 10.0, 10.0, 10, 10, 0, 0.0, testOdorants, 1920, 1080);
    //void MicroscopeManager::CreateImageManager(std::string imageManagerType, float minVol, float maxVol, int framesPerVolume, int volumesPerSecond, int laserMode, float scannerAmplitude, std::vector<char> odorants, int width, int height)
    imageManagerType = "HDF5";
